...
| step4-scope (5ea6514) | 206 / 206 | 183874 | 53030 | 36040 | 42320 | 30088 | scope-aware renamer: per-scope name reuse (shadowing), struct fields per struct, swizzle letters back in the pool; output re-resolved and binding compared (isomorphism self-check) |
| step5-bigram (9bc2c82) | 206 / 206 | 183874 | 53030 | 36088 | 42264 | 30144 | occurrence-weighted bigram scoring (default `--score count`); per-file sums ±0.1% on these tiny shaders, but −0.4..−0.6% on the concatenated stream and −1.3% gz / −1.5% zstd on the 4.8 KB raymarcher fixture (`--score freq` keeps the old order) |
| step6-simplify (e348391) | 206 / 206 | 183874 | 49091 | 35712 | 42100 | 29820 | rewrites: merge adjacent declarations/fields (−3757 raw, −317 gz alone), unwrap `{s}`, splice decl-free blocks, drop `in` params, `x=x+y`→`x+=y`, drop unary `+`, squeeze macro bodies / `#if`; each measured neutral-or-better on gz/xz/zstd; `--no-rewrite NAME` switches one off |
| step7-typenames (bcf7612) | 206 / 206 | 183874 | 49073 | 35717 | 42092 | 29803 | bugfix: struct type names reserved shader-wide (0.7.1-alpha renamed a type and a local to the same name, breaking every struct-using shader on real drivers); invariant checked on every crush. Size-neutral: −18 raw, +5 gz, −8 xz, −17 zst |
